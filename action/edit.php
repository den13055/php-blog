...
$result = $mysqli->query("SELECT * FROM `article` WHERE id = '$id' AND userId = '" . $user['id'] . "'");
...
if (count($_POST)) {
	...
	$mysqli->query("UPDATE `article` SET `title`='$title',`content`='$content' WHERE id = '$id' AND userId = '" . $user['id'] . "'");
	header("Location: /?act=articles");
	die();
}
